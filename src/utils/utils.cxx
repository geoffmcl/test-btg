...
bool in_world_range( double lat, double lon, double alt )
{
    if ( lat > 90.0 )
        return false;
    if ( lat < -90.0 )
...
bool bbox_in_world_range( PMBBOX pbb )
{
    if (in_world_range( pbb->max_lat, pbb->max_lon, pbb->max_alt ) &&
        in_world_range( pbb->min_lat, pbb->min_lon, pbb->min_alt ))
        return true;
...
char *get_mbbox_stg( PMBBOX pbb )
{
    char *cp = GetNxtBuf();
    MBBOX bb = *pbb;
    if (valid_mbbox( bb ) &&
        bbox_in_world_range(pbb) ) {
        sprintf(cp,"lat,lon,elev max %s,%s,%s min %s,%s,%s",
            double_to_stg(pbb->max_lat),
            double_to_stg(pbb->max_lon),
            double_to_stg(pbb->max_alt),
            double_to_stg(pbb->min_lat),
            double_to_stg(pbb->min_lon),
            double_to_stg(pbb->min_alt) );
    } else {
        strcpy(cp,"bbox NOT valid!");
    }
    return cp;
}
